...
if ($method eq 'POST') 
{
    ...
    if ($content_type =~ m|application/json|i) {
        ...
        if ($@) {
            ...
            print encode_json({status  => 'error',message => 'Invalid JSON body',raw     => $buffer});
            close $LOG;
            exit;
        }
    } else {
...
if (!$dbh) {
    print $log_fh "- Error: Database connection failed.\n";
    print JSON->new->allow_nonref->encode({ status => "error", message => "DB connection failed" });
    close($log_fh);
    exit(0);
}
...
if ($@) 
{
    ...
    print encode_json({ status => 'error', message => 'DB insert failed' });
    $dbh->disconnect;
    close $LOG;
    exit;
}
...
# --- Response ---
print encode_json({ status => 'success', message => 'Record inserted', received => \%params });
